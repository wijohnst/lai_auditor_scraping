...
const parseAddresses = () => {
	...
	const streetNumbersAll = livingstonAddresses.map(address => Number(address.Number));

	const minStreetNumber = 769; // 18th & Livingston
	const maxStreetNumber = 1937; // Livingston & Alum Creek

	const validStreetNumbers = streetNumbersAll.filter(streetNumber => streetNumber >= minStreetNumber && streetNumber <= maxStreetNumber);
